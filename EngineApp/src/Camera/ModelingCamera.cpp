...
void ModelingCamera::OnUpdate(float timestep, float width, float height)
{
	...
	if (!Hazel::Input::IsMouseButtonPressed(Hazel::Mouse::ButtonMiddle)) b_selected = false;
	...
	m_CameraModelMatrix = rot2 * rot1 * glm::mat4(1.0);

	if (height == 0.) height = 0.00001f;
	m_ProjectionMatrix = glm::perspective(glm::radians((float)m_FoV), (float)width / (float)height, 0.1f, 100.0f);
